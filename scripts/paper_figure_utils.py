...
def load_spec_curve(path: str | Path) -> pd.DataFrame:
    df = pd.read_csv(path)
...
def plot_spec_curve_from_csv(
    spec_curve_df: pd.DataFrame,
    outcome: str,
    term: str,
    ...
    """Specification curve for one outcome/term: sorted coefficients with 95% CI on
    top, and which confounder blocks (CONFOUNDER_BLOCKS) each specification included
    on the bottom. See REGRESSION_ROBUSTNESS_PLAN.md Sec. 3.3.
    """
    apply_paper_style()
    outcome_display = outcome_display or OUTCOME_DISPLAY
    term_labels = term_labels or TERM_LABELS
    block_labels = block_labels or {}

    sub = spec_curve_df[
        (spec_curve_df["outcome"] == outcome) & (spec_curve_df["term"] == term)
    ].copy()
    if sub.empty:
    ...
    ax_top.scatter(
        sub.loc[sig, "x"], sub.loc[sig, "coef"],
        s=26, facecolor=color, edgecolor="white", linewidth=0.6, zorder=4,
    )
    ax_top.scatter(
        sub.loc[~sig, "x"], sub.loc[~sig, "coef"],
        s=26, facecolor=HOLLOW_FACE, edgecolor=color, linewidth=1.0, zorder=4,
    )
    ...
    ax_bottom.set_yticklabels([block_labels.get(n, n) for n in reversed(block_names)], fontsize=9)
